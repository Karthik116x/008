export class CropMLService {
  ...
  async predictYield(predictionData: any) {
    try {
      ...
      return {
        expectedYield: prediction.cropYield,
        ...
        marketValue: await this.calculateMarketValue(prediction.cropYield, cropType),
        timestamp: new Date().toISOString()
      };
  ...
  private async runYieldPredictionModel(data: any) {
    ...
    return {
      cropYield: Math.round(totalYield * 100) / 100,
      range: {
  ...
  private calculateExpectedYield(crop: string, farmSize: number, environmental: any): string {
    ...
    let cropYield = baseYields[crop] || 20;
    
    // Adjust based on environmental factors
    if (environmental.soilNutrients.nitrogen > 75) cropYield *= 1.1;
    if (environmental.rainfall.annual < 500) cropYield *= 0.9;
    if (environmental.temperature.average > 35) cropYield *= 0.95;
    
    return `${Math.round(cropYield)} ${units[crop]}`;
  }
  ...
  private async calculateMarketValue(cropYield: number, cropType: string) {
    const prices = {
      tomatoes: 45, // ₹ per kg
    ...
    const totalValue = cropYield * price;
